...
import MobileChatBox from './molecules/MobileChatBox';
import { connect } from 'react-redux';
import axios from 'axios';
import { Fragment } from "react";
import { URL,getUsers,sortChats,getDate,getTime,datesAreOnSameDay,isUser,isImage,messageLength } from '../utils/allFunctions';
    ...
    render() {
        ...
        const chatChanged=async(chat)=>{
            try {
                ...
                document.getElementById("chat-mobile").classList.toggle("active")
                document.getElementById("chat-participants").classList.toggle("active")
                this.setState({allChats:allChatsDum,currentChat:chat,allMessages:response.data.messages,chatActive:true})
            } catch (error) {
        ...
        const closeChat=()=>{
            ...
            document.getElementById("chat-participants").classList.toggle("active")
            setTimeout(() => {
                this.setState({allMessages:[],currentChat:{}})
            }, 300);
        }
        ...
        const searchUserClicked=(_)=>{
            if(this.state.allUsers.includes(_._id)){
                ...
                document.getElementById("chat-mobile").classList.toggle("active")
                document.getElementById("chat-participants").classList.toggle("active")
                this.setState({allChats:allChatsDum,search_name:"",currentChat:newChat,allUsers:getUsers(allChatsDum),chatActive:true})
            }
        }
        return (
            <div className="d-flex m chat-parent">
                <div className="chat-top-level">
                    <div className="chat-participants" id="chat-participants">
                                    ...
                                    {this.state.allChats.length>0?
                                        <div className="chat-participants-body">
                                            {this.state.allChats.map((_, i) =>
                                                <div key={i} id={"participant"+i} className={this.state.currentChat?.chat?._id && this.state.currentChat.chat._id === _.chat._id ? "name" : "name"} onClick={(e)=>{
                                                    ...
                                                    <div className="profile-name">
                                                        ...
                                                        {_.chat?.last_message?.text?
                                                            <div className="row2">
                                                                ...
                                                                :messageLength( _.chat.last_message.text,32)}</p>
                                        ...
                                        :
                                        <div className="no-users">No chats available😥</div>
                                    }
                                    </Fragment>
                                :<Loading height="55"/>
                    ...
                    <div className="chat-box-mobile" id="chat-mobile">
                        {Object.keys(this.state.currentChat).length>0?
                            <MobileChatBox key={this.state.currentChat} closeChat={closeChat} newChat={newChat} addMessage={addMessage} currentChat={this.state.currentChat} allMessages={this.state.allMessages} socket={this.props.socket} user={this.props.user} />
                            :
